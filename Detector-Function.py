...
# creates output images folder/clears old one
...
# resizes output image to be smaller & adds gaussian blur
...
# stop sign detection function takes in a test image & returns output, true/false if it was detected, & number of inliers
def detect_stop_sign(test_img):

    # resizes test image & adds gaussian noise just like reference image
    max_dim = 500
    ...
    # extracts ORB features from test image
    ...
    # matches descriptors using brute force matching function and comparing hamming distances
    ...
    # sets inlier detection threshold
    if H is None or inliers < 9:
        output = test_img.copy()
        cv2.putText(output, "NOT DETECTED", (30, 50),
                    cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0,0,255), 3)
        cv2.putText(output, "Matches:" + str(inliers), (30, 90),
                    cv2.FONT_HERSHEY_SIMPLEX, 1.5, (0, 0, 255), 3)
    ...
    # creates a copy of the image and returns it as output
    output = test_img.copy()
...
# Processes through folders & computes accuracy
folders = {
...
# Loops through each folder and all images within the folder
for folder_name, folder_path in folders.items():
    ...
    # creates subfolder inside output-images
